...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "6586410d-530b-4aee-baf7-cd21fb37b75f"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.9724577244330825"
...
resource "aws_db_subnet_group" "default" {
  name       = "analyticsonaspdotnet-db-subnet"
...
#  name        = "analyticsonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application analyticsonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for analyticsonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "analyticsonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "analyticsonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_analyticsonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "analyticsonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/analytics-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/analyticsonaspdotnet theharbormaster/analytics-onaspdotnet:latest"
    ]
  }
}
